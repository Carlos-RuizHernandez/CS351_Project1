...
private:
  ...
  struct person {
    ...
    struct person *prev; // doubly linked list allows for easy access to
                         // neighbors in district
    int r;
    int c;
  };
  ...
  //
  void addToDistrict(world *&planet, district *&d, person *&p, int row,
                     int col) {
    // empty district
    if (d->pop == 0) {
...
public:
  ...
  GridWorld(unsigned nrows, unsigned ncols) {
    ...
    for (unsigned int x = 0; x < nrows; x++) {
      ...
      for (unsigned int y = 0; y < ncols; y++) {
        ...
        planetEarth->grid[x].push_back(newD);
      }
    }
  ...
  bool death(int personID) {
    ...
    if (personID < planetEarth->births) {
      ...
      if (markedForDeath->status == 1) {
        district *targetDistrict =
            planetEarth->grid[markedForDeath->r][markedForDeath->c];
  ...
  bool move(int id, int targetRow, int targetCol) {
    ...
    if (id < planetEarth->births && targetRow < planetEarth->numrows &&
      ...
      if (moving->status == 1) {
        ...
        addToDistrict(planetEarth, targetDistrict, moving, targetRow,
                      targetCol);
        return true;
      }
    }
   ...
   */
  int population() const { return planetEarth->pop; }
   ...
   */
  int num_rows() const { return planetEarth->numrows; }

  /*
   * function: num_cols
   * description:  returns number of columns in world
   */
  int num_cols() const { return planetEarth->numcols; }
};
